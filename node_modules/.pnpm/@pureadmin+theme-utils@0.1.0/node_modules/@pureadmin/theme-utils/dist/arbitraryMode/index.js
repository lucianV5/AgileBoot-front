...
exports.createSetCustomThemeFile = createSetCustomThemeFile;
exports.getThemeStyleContent = getThemeStyleContent;
...
function mix(color1, color2, weight) {
  ...
  const rgb = [arr1[0] * w1 + arr2[0] * w2, arr1[1] * w1 + arr2[1] * w2, arr1[2] * w1 + arr2[2] * w2, alpha];
  return _color.default.rgb(rgb);
}
function getMixColorAndWeight({
  primaryColor,
  targetColor,
  otherMixColors
}) {
  const arr = ['#ffffff', '#000000'].concat(otherMixColors || []).map(cstr => (0, _color.default)(cstr));
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    for (let j = 1; j <= 100; j++) {
      ...
      if (Math.round(arr1[0]) === Math.round(arr2[0]) && Math.round(arr1[1]) === Math.round(arr2[1]) && Math.round(arr1[2]) === Math.round(arr2[2])) {
        weight = j;
        mixColor = color1;
        break;
      }
...
function getHsvPercentGias(primaryColor, resultColor) {
  ...
  const getGiasPercent = index => primaryHsvArr[index] !== 0 ? (primaryHsvArr[index] - resultHsvArr[index]) / primaryHsvArr[index] : primaryHsvArr[index];
  return [getGiasPercent(0), getGiasPercent(1), getGiasPercent(2), primaryColor.valpha !== 0 ? (primaryColor.valpha - resultColor.valpha) / primaryColor.valpha : primaryColor.valpha];
}
function separatValue(cssValues) {
  ...
  cssValues.forEach(val => {
    ...
    const hex = val.match(_utils2.colorValueReg.hex);
    const rgb = val.match(_utils2.colorValueReg.rgb);
    const rgba = val.match(_utils2.colorValueReg.rgba);
    const hsl = val.match(_utils2.colorValueReg.hsl);
    const hsla = val.match(_utils2.colorValueReg.hsla);
      ...
      const getResidueVal = (colorArr, cssColors, residueVal) => {
        ...
        if (colorArr) {
          colorArr.forEach(cval => {
            cssColors.push(cval);
            newVal = residueVal.replace(cval, hasSpace ? '#{color}' : '');
          });
        }
        return newVal;
      };
...
function getResultColorReplaceMap({
  ...
  try {
    primaryVarColor = (0, _color.default)(defaultPrimaryColor).hsv();
  } catch (e) {
    throw Error(`error:defaultPrimaryColor: ${defaultPrimaryColor}  not a color value`);
  }
  const hueDiffControler = {
    low: 0,
    high: 0,
    ...(hueDiffControls || {})
  };
  ...
  cssColors.forEach(colorString => {
    ...
    if (includeStyleWithColors.filter(item => !item.inGradient).some(item => {
      if (Array.isArray(item.color)) {
        return item.color.some(co => (0, _utils2.isSameColor)(co, colorString));
      }
      return (0, _utils2.isSameColor)(item.color, colorString);
    })) {
      return;
    }
    const resultColor = (0, _color.default)(colorString).hsv();
    let finded = false;
    for (let index = 0; index < varsColorValues.length; index++) {
      const varStr = varsColorValues[index];
      const varColor = (0, _color.default)(varStr).hsv();
...
function getThemeStyleContent(scopeName, removeCssScopeName) {
  return (0, _utils.extractThemeCss)({
    scopeName,
    removeCssScopeName
  }).then(({
    themeCss,
...
function createSetCustomThemeFile({
  ...
  if (_fsExtra.default.existsSync(`${targetRsoleved}/baseVarColors.json`)) {
    baseVarColorsJson = JSON.parse(_fsExtra.default.readFileSync(`${targetRsoleved}/baseVarColors.json`).toString());
  }
  const {
    cssColors,
    hybridValueMap,
  ...
  const targetValueReplacer = Object.keys(sourceColorMap).concat(Object.keys(hybridValueMap)).concat(otherValues).reduce((tol, curr) => {
    return {
      ...tol,
      [curr]: curr
    };
  }, {});
  const gradientReplacer = Object.keys(sourceColorMap).map(key => sourceColorMap[key].sourceVarColorString).reduce((tol, curr) => {
    return {
      ...tol,
      [curr]: curr
    };
  }, {});
  const browerCodes = _fsExtra.default.readFileSync(_path.default.join(__dirname, './browser.js')).toString();
  _fsExtra.default.outputFile(`${targetRsoleved}/customThemeOptions.json`, JSON.stringify({
  ...
  const fileContent = `
${importUtils ? 'import { getSetNewThemeMethod } from "@pureadmin/theme-utils/dist/arbitraryMode/browser";' : ''}
 ${!importUtils ? browerCodes.replace(/import\s+([^\s]+|\{[^{}]+\})\s+from ['"][^'"]+['"];?/g, '').replace(/export\s+\{[^{}]+\};?/g, '') : ''}\n    
 ...
 * @param {object} [options.gradientReplacer=${JSON.stringify(gradientReplacer)}] 存在多个梯度主色，可对应替换梯度主色
 * @param {object} [options.targetValueReplacer=${JSON.stringify(targetValueReplacer)}] 可用于非颜色值的替换，如"padding:10px;" 中的 "10px"，（如果是颜色值，则是精确替换颜色）
 */
function setCustomTheme(options) {
  ...
  return _prettier.default.resolveConfig(process.cwd()).then(options => {
    const setCustomThemeConent = _prettier.default.format(`${preAppendedContent || ''}${fileContent}${appendedContent || ''}`, {
      ...(options || {}),
      parser: 'babel'
    });
    if (customThemeOutputPath) {
      _fsExtra.default.outputFileSync(customThemeOutputPath, setCustomThemeConent);
    }
    return {
      styleContent,
