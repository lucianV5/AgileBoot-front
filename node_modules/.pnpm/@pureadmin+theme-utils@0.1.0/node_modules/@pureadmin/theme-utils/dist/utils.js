...
const getScropProcessResult = exports.getScropProcessResult = getScopeProcessResult;
/**
 *
 * @param {String} url
...
function createArbitraryModeVarColors(filecontent) {
  if (filecontent) {
    ...
    const hsla = (filecontent.match(_utils.colorValueReg.hsla) || []).map(color => color.replace(/\s+/g, '').replace(/,0(?=,)/g, ',0%'));
